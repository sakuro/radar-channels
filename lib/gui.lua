...
local function build_gui(player)
  ...
  local elems, window = gui_add(player.gui.screen, {
    ...
    style_mods = { minimal_width = MAIN_WIDTH },
    {
      ...
      drag_target = WINDOW_NAME,
      {
        type = "label",
        style = "frame_title",
        caption = { "gui.radar-channels-title" },
        ignored_by_interaction = true,
      },
      {
        type = "empty-widget",
        ...
        drag_target = WINDOW_NAME,
        style_mods = { horizontally_stretchable = true, right_margin = 4 },
      },
      {
        type = "sprite-button",
  ...
  t.add({
    ...
    caption = {
      "",
      { "gui.radar-channels-radars" },
      " (",
      { "gui-control-behavior.mode-of-operation" },
      ": ",
      { "gui-control-behavior-modes-guis.radar-universe" },
      ")",
    },
  })
